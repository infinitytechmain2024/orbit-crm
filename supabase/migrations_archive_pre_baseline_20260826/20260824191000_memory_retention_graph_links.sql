...
begin
  ...
  insert into public.graph_nodes(
    ...
    org_id, tg_argv[0], row_id,
    coalesce(row_data ->> 'business_name', row_data ->> 'title'),
    row_data ->> 'status',
    case when tg_argv[0] = 'client'
      then jsonb_build_object('category', row_data ->> 'category', 'city', row_data ->> 'city_location')
      else jsonb_build_object('starts_at', row_data ->> 'starts_at', 'ends_at', row_data ->> 'ends_at')
    end,
    true, actor_id
  )
  ...
  if tg_argv[0] = 'calendar_event' then
    ...
    if client_id is not null then
      select id into linked_node_id from public.graph_nodes
       where organization_id = org_id and entity_type = 'client' and entity_id = client_id;
      if linked_node_id is not null then
        insert into public.graph_relations(
          ...
          org_id,'calendar_event',row_id,'client',client_id,'for_client',
          event_node_id,linked_node_id,'one_way',true,actor_id
        ) on conflict (organization_id,source_node_id,target_node_id,relation_type,direction) do nothing;
      end if;
    end if;
    if task_id is not null then
      select id into linked_node_id from public.graph_nodes
       where organization_id = org_id and entity_type = 'task' and entity_id = task_id;
      if linked_node_id is not null then
        insert into public.graph_relations(
          organization_id,source_type,source_id,target_type,target_id,relation_type,
          source_node_id,target_node_id,direction,is_automatic,created_by
        ) values (
          org_id,'calendar_event',row_id,'task',task_id,'for_task',
          event_node_id,linked_node_id,'one_way',true,actor_id
        ) on conflict (organization_id,source_node_id,target_node_id,relation_type,direction) do nothing;
      end if;
    end if;
