...
begin
  ...
  return case target_type
    ...
    when 'person' then exists (
      ...
      select 1 from public.ai_agents agent
      where agent.organization_id = target_organization_id and agent.id = target_id
    )
    when 'client' then exists (
      select 1 from public.leads lead
      where lead.organization_id = target_organization_id and lead.id = target_id
    )
    when 'document' then exists (
      select 1 from public.files file
      where file.organization_id = target_organization_id and file.id = target_id
    ) or exists (
      select 1 from public.artifacts artifact
      where artifact.organization_id = target_organization_id and artifact.id = target_id
    )
    ...
    when 'comment' then exists (
      ...
      where comment.organization_id = target_organization_id and comment.id = target_id
    )
    when 'finance_transaction' then exists (
      select 1 from public.finance_transactions item
      where item.organization_id = target_organization_id and item.id = target_id
    )
    when 'approval' then exists (
      select 1 from public.approval_requests request
      where request.organization_id = target_organization_id and request.id = target_id
    )
    when 'task_label' then exists (
      select 1 from public.task_labels label
      where label.organization_id = target_organization_id and label.id = target_id
    )
...
-- Compatibility/read-model views use invoker security so underlying RLS remains active.
...
with (security_invoker = true)
as
  select organization_id, id, 'project'::text as node_type, name as title,
         status::text, created_at, jsonb_build_object('color', color) as metadata
  from public.projects
  union all
  select organization_id, id, 'task'::text, title, status, created_at,
         jsonb_build_object('project_id', project_id, 'workflow_run_id', workflow_run_id)
  from public.ai_tasks
  union all
  select organization_id, id, 'person'::text, name, status, created_at,
         jsonb_build_object('role', role, 'capabilities', capabilities)
  from public.ai_agents
  union all
  select organization_id, id, 'document'::text, name, type, created_at,
         jsonb_build_object('task_id', task_id, 'project_id', project_id, 'url', url)
