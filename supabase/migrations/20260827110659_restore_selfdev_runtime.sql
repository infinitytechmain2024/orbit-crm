-- Restore the controlled self-development runtime. The original migration was
-- recorded in production history without its objects being present, so this
-- migration is intentionally idempotent.

create table if not exists public.development_providers (
  ...
  active_runs integer not null default 0 check (active_runs between 0 and 8),
...
create table if not exists public.development_run_events (
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and policyname = 'development_providers_member_select') then
    create policy development_providers_member_select on public.development_providers
      for select to authenticated
      using (private.is_organization_member(organization_id, (select auth.uid())));
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'public' and policyname = 'development_runs_member_select') then
    create policy development_runs_member_select on public.development_runs
      for select to authenticated
      using (private.is_organization_member(organization_id, (select auth.uid())));
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'public' and policyname = 'development_run_events_member_select') then
    create policy development_run_events_member_select on public.development_run_events
      for select to authenticated
      using (private.is_organization_member(organization_id, (select auth.uid())));
  end if;
end
